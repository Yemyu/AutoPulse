...
> `docs/数据说明.md` (Chinese data description) and `data/sentiment/data_quality_report.md`
> (sentiment quality report) have been merged into this file to avoid fragmentation.
...
pure `requests`, no browser, no login. Crawler: `scripts/sentiment_crawler.py` (v7, supports
...
> Regenerate summary / quality report: `python scripts/build_sentiment_summary.py` (reads
> `sentiment_reviews.csv`, outputs `sentiment_summary.csv` to `data/sentiment/`, and
> `data_quality_report.json` / `.md` to `docs/`, reproducible).
...
> `scripts/build_sentiment_summary.py`.
...
Generated by `scripts/clean_and_align.py` — the aligned analysis input after joining all three tables:
...
- **Current status** (full crawl done): 490 series → 489 matched sales, ready for regression; rerun `clean_and_align.py` to refresh with new data.

---

## 8. Sentiment Data Quality & Coverage Report (Full)

> The generation logic lives in `scripts/build_sentiment_summary.py`; rerunning outputs
> `docs/data_quality_report.json` and `docs/data_quality_report.md` (reproducible). The
