...
| `sentiment_reviews.csv` | Dongchedi review API | `sentiment_crawler.py` | User review details (rating + text), paginated, resumable |
| `sentiment_summary.csv` | aggregated from reviews | `build_sentiment_summary.py` | Series-level sentiment metrics |
...
python scripts/sentiment_crawler.py --all --max 100
...
├── scripts/
│   ├── sentiment_crawler.py        # review crawler v7 (resumable)
│   ├── build_sentiment_summary.py  # summary + quality report
│   └── clean_and_align.py          # clean + align three tables -> analysis_input.csv
...
- [x] Data cleaning & three-table alignment (`clean_and_align.py` → `analysis_input.csv`, 489 series with sentiment + sales + spec)
