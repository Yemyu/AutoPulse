...
| `sentiment_reviews.csv` | 懂车帝口碑 API | `sentiment_crawler.py` | 用户口碑明细（评分 + 文本），逐页采集、断点续传 |
| `sentiment_summary.csv` | 由 reviews 聚合 | `build_sentiment_summary.py` | 车系级情感指标 |
...
├── scripts/
│   ├── sentiment_crawler.py        # 舆情采集器 v7 (断点续传)
│   ├── build_sentiment_summary.py  # 汇总 + 质量报告生成
│   └── clean_and_align.py          # 清洗 + 三表对齐 -> analysis_input.csv
...
- [x] 数据清洗与三表对齐（`clean_and_align.py` 产出 analysis_input.csv，489 系具备舆情+销量+配置）
